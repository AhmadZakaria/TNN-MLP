  ...
  public:
    ...
    Eigen::MatrixXd activate(Eigen::MatrixXd& mat, bool deriv = false) {
        if(!deriv)
            ...
            switch (activation) {
            case TANH:
                return mat.unaryExpr(&tanh_deriv);
                break;
            case LOGISTIC:
                return mat.unaryExpr(&logistic_deriv);
                break;
            case IDENTITY:
                return mat;
                break;
    ...
    MLPLayer addGradToW(Eigen::MatrixXd &value) {
        W += value;
        return *this;
    }
    ...
    void init() {
        Z.resize(batch_size, num_neurons);
        S.resize(batch_size, num_neurons);
        W.resize(num_neurons, num_outputs);
        D.resize(batch_size, num_neurons);
        F.resize(num_neurons,batch_size);
